...
- [ ] 署名検証のユニットテスト: 正しい署名のリクエストを通し、署名が不正・欠落のリクエストを拒否する
- [ ] エコー応答ロジックのユニットテスト(受信テキスト→返信メッセージの組み立て)
- [ ] GitHub Actionsで push ごとに lint+ユニットテストが自動実行される(CI)

## 完了条件

- [ ] LINEでボットにテキストを送ると、同じ文字がそのまま返ってくる(手動確認)
- [ ] Terraform apply がS3バックエンド+2プロバイダ構成で通る
- [ ] GitHub ActionsからOIDCでAWSにデプロイできる
- [ ] CIでlint+ユニットテストが通っている

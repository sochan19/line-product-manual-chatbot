...
| `docs/_docmap.json` | ドキュメント契約。各ドキュメントの役割・所有トピック(SSoT)・上限行数を宣言。`pnpm docs:check` と Skill `doc-consistency-review` が参照する |
...
3. **テストはspecとセットで書く**: 各specの「テスト」項を実装と同時に満たす。完了条件はできる限り自動テスト(ユニットテスト+評価セット)に落とし込み、CIで毎push実行する。実APIが絡む部分はポートをモックに差し替えてロジックを検証し、実APIとの結合は手動確認タスクとして分ける。テストの書き方(ケースごとの概要コメント・対象コードの実行順に並べる)はルール11に従う。
...
10. **ドキュメントを書き換えたら整理レビューをする**: 仕様変更の反映後・フェーズ完了前(`phase-error-review` とセット)・`docs/**` を変更するPRを出す前に、Skill `doc-consistency-review`(`.claude/skills/doc-consistency-review/SKILL.md`)を実行し、ドキュメント群の整合性・重複・肥大化・役割適合を確認する(目的はAIのコンテキスト消費を増やさないこと=1事実1箇所+リンク)。機械的なチェック(サイズ上限・リンク切れ・見出し重複・追跡漏れ)は `pnpm docs:check`(CIでも毎push実行、NGでビルド失敗)。ドキュメントの役割・所有トピック・上限行数は `docs/_docmap.json` が契約で、変更するときはコードより先にここと本ドキュメントマップを直す。
11. **コードを書いたらコメント・並び順をレビューする**: テストコードや実装コードを追加・変更したら、Skill `code-comment-review`(`.claude/skills/code-comment-review/SKILL.md`)を実行し、(1) 各テストケース(`it`)の上にテスト内容の概要コメントがあるか、(2) 同じファイル内のテストがテスト対象コードの分岐実行順(ガード節・早期returnが上、最後に実行される処理が下)に並んでいるか、(3) 実装・テストを問わず各関数の上に何のための関数かのコメントがあるか、を確認する。フェーズ完了前は `phase-error-review` とセットで実行する。
